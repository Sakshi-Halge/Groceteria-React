...
class GetInfo extends Component {
  ...
  placeOrder = () => {
    ...
    myOrderInfo.productName = this.props.productName;
    myOrderInfo.amount = this.props.totalPrice;
    myOrderInfo.address = item.address;
    myOrderInfo.status = item.status;
    myOrderInfo.productImg = this.props.productImg;
    myOrderInfo.quantity = this.props.quantity;
    myOrderInfo.status = item.status;
  ...
  handleSubmit = () => {
    ...
    fetch(`${viewcart}${email}?id=${this.props.id}`)
      .then((res) => res.json())
      .then((data) => {
        if (data.length > 0) {
          let query = { id: this.props.id };
          fetch(`${deleteApi}`, {
            method: "DELETE",
            headers: {
              accept: "application/json",
  ...
  renderContent = () => {
    if (sessionStorage.getItem("userData")) {
      return (
        <>
          <form method="POST" action="https://edupayment.herokuapp.com/paynow">
    ...
    return (
      <>
        <div className={`info-section ${this.props.payClassName}`}>
          <div className="info-sec-box">{this.renderContent()}</div>
